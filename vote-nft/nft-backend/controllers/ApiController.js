...
const SingleNFTAbi = JSON.parse(fs.readFileSync(`./abis/SingleNFT.json`));
...
module.exports = BaseController.extend({
  ...
  createProposal: async function (req, res, next) {
    ...
    var balance = 0;
    if (spaceEntity.nftType == 0) {
      // ERC721
      const rpc = getNodeUrl(spaceEntity.chainId);
      const provider = new ethers.providers.JsonRpcProvider(rpc);
      const nftContract = new ethers.Contract(spaceEntity.nftAddr, SingleNFTAbi, provider);
      balance = await nftContract.balanceOf(address);
    } else {
      // ERC1155
      try {
        // get balance from subgraph
        const subgraph_link = config.contracts[spaceEntity.chainId]?.subgraph;
        const balanceID = `${spaceEntity.nftAddr}-${address}`;
        ...
        if (result.data.data && result.data.data.balance) {
          balance = result.data.data.balance.value;
        }
      } catch (error) {
        
      }
    }
    if (Number(balance) < spaceEntity.createLimit) {
      return res.status(404).send({ message: `You need to have a minimum of ${spaceEntity.createLimit} entities in order to submit a proposal.` });
    }
  ...
  createVote: async function (req, res, next) {
    ...
    if (spaceEntity.nftType == 0) {
      ...
      const provider = new ethers.providers.JsonRpcProvider(rpc);
      const nftContract = new ethers.Contract(spaceEntity.nftAddr, SingleNFTAbi, provider);
      power = await nftContract.balanceOf(address);
    } else {
      // ERC1155
      try {
        // get balance from subgraph
        ...
        const result = await axios(subgraph_config);
        if (result.data.data && result.data.data.balance) {
          power = result.data.data.balance.value;
        }
      } catch (error) {        
      }
    }
    ...
    if (Number(power) <= 0) {
      return res.status(404).send({ message: `You do not own nft` });
    }
