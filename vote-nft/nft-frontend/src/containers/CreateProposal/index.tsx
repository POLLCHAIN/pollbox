...
const CreateProposal = (props: any) => {
  ...
  useEffect(() => {
    if (account && chainId && spaceInfo) {
      getNFTBalance(account, spaceInfo.nftAddr, spaceInfo.nftType, chainId, library?.getSigner())
        .then((balance) => {
  ...
  async function onCreateProposal() {
    ...
    if (nftBalance < spaceInfo.createLimit) {
      toast.error(`You don't hold enough entities`);
      return;
    }
          ...
          <div className={classes.section}>
            <div className={classes.rootContent}>
              <div className="text-left mx-auto my-10 w-5/6 sm:max-w-xl md:max-w-2xl lg:max-w-3xl xl:max-w-5xl">
                ...
                <div>
                  <p className="">
                    You need to have a minimum of {spaceInfo.createLimit} entities in order to submit a proposal.
                  </p>
                  <h4 className="text-cusBlack text-lg">
                    Your Balance : <span className='font-bold'>{nftBalance} entities</span>
                  </h4>
                </div>
