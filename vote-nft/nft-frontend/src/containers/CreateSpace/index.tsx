...
import { isNFTAddress, getCreatePrice, createSpace } from '../../utils/contracts';
...
const CreateSpace = () => {
  ...
  async function onCreateSpace() {
    ...
    if (balance < Number(priceInfo.price)) {
      toast.error("Insufficiant funds!");
      return;
    }

    const nftStatus = await isNFTAddress(nftAddr, chainId, library?.getSigner());
    if (!nftStatus) {
      toast.error("Please Input NFT Collection Address Correctly!");
      return;
    }
          ...
          <div className={classes.section}>
            <div className={classes.rootContent}>
              <div className="text-left mx-auto my-10">
                ...
                </h1>
                {
                  ...
                  <div>
                    ...
                    <p className="text-cusBlack text-lg">
                      Your Balance : <span className='font-bold'>{formatNum(balance)} {priceInfo.symbol}</span>
                    </p>
                  </div>
                }
                ...
                <div className="mt-8">
                  <h4 className="mr-5 text-cusBlack font-bold text-lg">NFT Collection Contract <span className="text-cusLink">*</span></h4>
                  <input type="text" name="" id="" placeholder='e.g 0x32A2E3cA31210Bed60D60B16745902Cc6bD37e90'
                    className="bg-transparent w-full h-full  text-cusBlack
                                        lightBlueBg p-5 sm:p-6 rounded-xl mt-3 focus:outline outline-blue-500"
                    value={nftAddr} onChange={event => setNftAddr(event.target.value)} />
                </div>

                {/* create limit */}
                <div className="mt-8">
                  <h4 className="mr-5 text-cusBlack font-bold text-lg">NFT holding limit to allow to submit proposal <span className="text-cusLink">*</span></h4>
                  <input type="number" name="" id="" placeholder=''
                    className="bg-transparent w-full h-full  text-cusBlack
                                        lightBlueBg p-5 sm:p-6 rounded-xl mt-3 focus:outline outline-blue-500"
                    value={createLimit} onChange={event => setCreateLimit(Number(event.target.value))} />
                </div>
